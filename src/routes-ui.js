function renderRouteCards() {
  ...
  for (const r of routes) {
    ...
    card.innerHTML = `
      ...
      <button class="route-exec-btn" data-route="${r.id}" style="width:100%;padding:6px 10px;background:rgba(${parseInt(r.color.slice(1,3),16)},${parseInt(r.color.slice(3,5),16)},${parseInt(r.color.slice(5,7),16)},0.12);color:${r.color};border:1px solid ${r.color}99;cursor:pointer;font-family:'Courier New',monospace;font-size:11px;letter-spacing:2px">&#9654; EXECUTE</button>
    `;
